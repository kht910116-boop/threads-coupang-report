...
CLI가 없는 서비스는 백그라운드 브라우저로 돌린다. **작업 중에 창이 뜨거나 포커스를
뺏지 않는다.**

- 프로바이더마다 **전용 브라우저 프로파일**(`data/browser-profiles/<id>`)을 쓴다.
  평소 쓰는 브라우저는 건드리지 않는다.
- 실행은 **헤드리스**다. 화면에 아무것도 안 뜬다.
- 창이 뜨는 건 **최초 1회 로그인할 때뿐**이다. 연결 상태 화면에서 '로그인'을 누르면
  창이 열리고, 직접 로그인하면 세션이 프로파일에 남는다. 그 뒤로는 백그라운드.
- 같은 프로파일에서 **세션 쿠키를 꺼낼 수 있다**('쿠키' 버튼). 쿠키 방식으로 직접
  호출할 때 로그인을 두 번 하지 않아도 된다.

완료 판정은 사이트별 '생성 완료' 표시를 찾지 않고, **답변 텍스트가 일정 시간
변하지 않으면 끝난 것으로 본다.** 어느 사이트에서든 통하고 UI가 바뀌어도 안 깨진다.

선택자는 `data/web-providers.json`(화면에서 편집)에 있다.

```jsonc
{
  "id": "chatgpt",
  "url": "https://chatgpt.com/",
  "promptSelector": "#prompt-textarea",   // textarea / contenteditable 둘 다 됨
  "submit": "enter",                      // "enter" 또는 클릭할 버튼 선택자
  "responseSelector": "[data-message-author-role='assistant']",
  "stableMs": 2500,                       // 이만큼 안 변하면 완료
  "loggedInSelector": "#prompt-textarea"  // 이게 보이면 로그인된 상태
}
```

**기본 레시피의 선택자는 전부 미검증이다.** 이 저장소에서는 해당 사이트에 접속할 수
없어 확인할 방법이 없었다. 각자 환경에서 '시험' 버튼으로 확인하고 고쳐야 한다.
드라이버 자체(타이핑·전송·완료 감지·추출)는 로컬 테스트로 검증했다.

> 참고: 로그인된 웹 UI를 자동으로 조작하는 것은 대부분 서비스의 약관에서 금지 항목이다.
> 본인 계정·개인 용도라도 계정 제재 가능성은 남는다.

### 엔진 우선순위

설치·로그인된 것을 위에서부터 찾아 쓴다: **CLI → 웹 자동화 → 종량제 API**.
CLI가 먼저인 이유는 빠르고, 화면 변경에 안 깨지고, 구조화 출력을 지원하기 때문이다.
`PLANNER_AGENT=<id>`로 고정할 수 있다.
...
src/lib/
  ...
  engine/               기획 엔진 — 주제+프리셋 → 컷 단위 기획서
    prompt.ts             세 경로가 공유하는 프롬프트
    agents.ts             구독 CLI 레지스트리 (설정으로 CLI를 붙인다)
    cli.ts                CLI 범용 러너 — CLI 이름을 모른다
    web.ts                구독 웹 경유 (브라우저 자동화)
    api.ts                Anthropic SDK 경유 (종량제)
    index.ts              설치·로그인된 것을 우선순위대로 선택
  providers/web/
    recipes.ts            사이트별 선택자 레시피
    driver.ts             백그라운드 브라우저 (전용 프로파일, 헤드리스, 쿠키 추출)
  store.ts              파일 기반 저장소
...
| 부분 | 상태 |
|---|---|
| 구독 CLI 경로 실제 기획 생성 | 통과 — 19컷 / 46.2초, 프리셋 제약 전부 준수 |
| 웹 자동화 드라이버 (타이핑·전송·완료 감지·추출) | 통과 — 로컬 테스트 사이트로 textarea·contenteditable 양쪽 확인 |
| 웹 레시피 선택자 (ChatGPT / Gemini / Grok …) | **미검증** — 해당 사이트 접속이 막혀 확인 불가. '시험' 버튼으로 각자 확인 필요 |
